"""modified from https://github.com/CNChTu/Diffusion-SVC/blob/main/preprocess.py"""
import argparse
import os
import random
import shutil
...
def preprocess(path, f0_extractor, volume_extractor, mel_extractor, units_encoder, sample_rate, hop_size,
  ...
  def process(file):
    binfile = file + '.npy'
    path_srcfile = os.path.join(path, file)
  ...
  print('Preprocess the audio clips in :', path)

  # single process
  filelist = [x for x in os.listdir(path) if os.path.splitext(x)[1][1:] in extensions]
