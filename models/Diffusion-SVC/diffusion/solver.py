...
def test(args, model, vocoder, loader_test, saver):
    ...
    # run
    with torch.no_grad():
        for bidx, data in enumerate(loader_test, 1):
            fn = data['name'][0]
            print('--------')
            ...
            # log audio
            path_audio = os.path.join(args.data.valid_path, data['name_ext'][0])
...
def train(args, initial_global_step, model, optimizer, scheduler, vocoder, loader_train, loader_test):
    ...
    for epoch in range(args.train.epochs):
        for batch_idx, data in enumerate(loader_train):
            ...
            # validation
            if saver.global_step % args.train.interval_val == 0 and saver.global_step > args.train.eval_delay:
                optimizer_save = optimizer if args.train.save_opt else None

                # save latest
                saver.save_model(model, optimizer_save, postfix=f'{saver.global_step}')

                # maybe delete oldest ckpt
                ckpts = saver.get_checkpoints()

                if len(ckpts) > args.train.num_ckpts:
                    min_skip_step = min([int(re.search('[0-9]*.pt', x).group()[:-3]) for x in ckpts])
                    saver.delete_model(postfix=str(min_skip_step))
