...
if __name__ == '__main__':
    ...
    # maybe copy `model_0.pt`
    os.makedirs(args.env.expdir, exist_ok=True)
    ckpts = [x for x in os.listdir(args.env.expdir) if x.endswith('.pt')]
    if not ckpts:
      import shutil
      print("Copying `model_0.pt` from `./checkpoints/shallow-diff`..")
      shutil.copy('./checkpoints/shallow-diff/model_0.pt', args.env.expdir)
