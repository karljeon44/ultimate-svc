...
def infer(
    ...
    """Inference"""
    from inference.main import infer
...
def pre_resample(
    ...
    from preprocessing.preprocess_resample import preprocess_resample
...
@click.option(
    ...
    "--train-dir",
    type=str,
    default=Path("./DATA/sovitz/train"),
    help="path to training dir (after preliminary prp)",
)
@click.option(
    "-d",
    "--dev-dir",
    type=str,
    default=Path("./DATA/sovitz/dev"),
    help="path to dev dir (after preliminary prp)",
)


def pre_flist(train_dir: Path, dev_dir: Path):
    """Preprocessing part 2: config"""
    from models.so_vits_svc_fork.preprocess_flist_config import preprocess_flist

    preprocess_flist(train_dir, dev_dir)
...
@click.option(
    ...
    type=click.Path(exists=True),
    help="path to config",
    default=Path("./configs/sovits.json"),
)
@click.option(
    "-n",
    "--n-jobs",
...
def pre_sd(
    ...
    from preprocessing.preprocess_speaker_diarization import preprocess_speaker_diarization
...
def pre_split(
    ...
    from preprocessing.preprocess_split import preprocess_split
